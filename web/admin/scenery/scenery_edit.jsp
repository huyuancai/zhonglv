...
<script type="text/javascript">
	$().ready(function(){
		...
		$("#Button1").click(function(){
			...
			var scenery_id = $("input[name='scenery_id']").val();
			...
			var data = {};
			data['sceneryVO.sceneryId']=scenery_id;
...
<body>
	<table style="width:100%;" border="0" cellpadding="0" cellspacing="1" class="black_border">
		<tbody>
			<tr class="suphead">
		        <td colspan="2">编辑景点</td>
		    </tr>
		    <tr>
      			<td class="fields_text">景点名称：</td>
      			<td class="fields_input">
          			<input name="scenery_name" type="text" value="<s:property value="#request.sceneryVO.getSceneryName()"/>" id="scenery_name" class="input_long">
          			<input name="scenery_id" type="hidden" value="<s:property value="#request.sceneryVO.getSceneryId()"/>" id="scenery_id">
          			<span id="RequiredFieldValidator1" style="color:Red;display:inline-block;">(*)</span>
	                &nbsp;级别：
	                <select name="scenery_level" id="scenery_level">
						<option value="AA" <s:if test="#request.sceneryVO.getSceneryLevel() == 'AA'"> selected</s:if>>AA</option>
						<option value="AAA" <s:if test="#request.sceneryVO.getSceneryLevel() == 'AAA'"> selected</s:if>>AAA</option>
						<option value="AAAA" <s:if test="#request.sceneryVO.getSceneryLevel() == 'AAAA'"> selected</s:if>>AAAA</option>
						<option value="AAAAA" <s:if test="#request.sceneryVO.getSceneryLevel() == 'AAAAA'"> selected</s:if>>AAAAA</option>
					</select>
                </td>
    		</tr>
    		<tr>
		     	...
		     	<td class="fields_input">
		     		<input name="scenery_title" type="text" value="<s:property value="#request.sceneryVO.getSceneryTitle()"/>" id="scenery_title" class="input_long">
		     	</td>
		   	</tr>
		   	<tr>
		      ...
		      <td class="fields_input">
		        ...
		        <input name="scenery_thumb" value="<s:property value="#request.sceneryVO.getSceneryThumb()"/>" id="scenery_thumb" type="hidden" />
      			...
      			<td class="fields_input">          
          			<select name="scenery_city" id="scenery_city">
          				...
          				<s:iterator var="destinationCityVO" value="#request.destinationCityVOs">
				            <option value="<s:property value="#destinationCityVO.getDesCityId()"/> " <s:if test="#destinationCityVO.getDesCityId() == #request.sceneryVO.getSceneryCity() "> selected</s:if>><s:property value="#destinationCityVO.getDesCityName()"/> </option>
          				</s:iterator>
			        </select>
					&nbsp;地址 ：<input name="scenery_addr" value="<s:property value="#request.sceneryVO.getSceneryAddr()"/>" type="text" id="scenery_addr" class="input_middle">
	          		&nbsp;地图坐标：<input name="scenery_lng" type="text" value="<s:property value="#request.sceneryVO.getSceneryLng()"/>" readonly="readonly" value="0.00" id="scenery_lng" class="input_normal">
		          <input name="scenery_lat" type="text" value="<s:property value="#request.sceneryVO.getSceneryLat()"/>" readonly="readonly" value="0.00" id="scenery_lat" class="input_normal">
		    ...
		    <tr>
      			...
      			<td class="fields_input">
					<input name="scenery_price" type="text" value="<s:property value="#request.sceneryVO.getSceneryPrice()"/>" id="scenery_price" class="input_mini" style="width:51px;">
          			元<span id="RegularExpressionValidator1" style="color:Red;visibility:hidden;">限数字</span>
          			&nbsp;取票地址：<input name="scenery_tickets_addr" type="text" value="<s:property value="#request.sceneryVO.getSceneryTicketsAddr()"/>" id="scenery_tickets_addr" class="input_middle">
         			 &nbsp;电话：<input name="scenery_tel" type="text" value="<s:property value="#request.sceneryVO.getSceneryTel()"/>" id="scenery_tel" class="input_normal">
          		</td>
    		</tr>
    		<tr>
		    	<td class="fields_text">摘要：</td>
		      	<td class="fields_input">
		        	<textarea name="scenery_summary" rows="2" cols="20" id="scenery_summary" class="text_content"><s:property value="#request.sceneryVO.getScenerySummary()"/></textarea>
		      	</td>
		    </tr>
    		<tr>
		      ...
		      <td class="fields_input">
		        <div class="ke-container ke-container-default" style="width: 100%;">
		        	<div style="display: block; height: 238px;" class="ke-edit">
		        		<textarea class="ke-edit-textarea" name="scenery_introduce" hidefocus="true" style="width: 100%; height: 238px;"></textarea>
		        		<script>
							var editor1;
							KindEditor.ready(function(K) {
								editor1 = K.create('textarea[name="scenery_introduce"]', {
								...
								editor1.html(HTMLDecode('<s:property value="#request.sceneryVO.getSceneryIntroduce()"/>'));
							});
						</script>
		        	</div>
			    </div>
		    ...
		    <tr>
		      ...
		      <td align="left" height="30" class="fields_input">
                <s:if test="#request.sceneryVO.isSceneryRecommend() == true">
                	<input id="scenery_recommend" type="checkbox" name="scenery_recommend" checked="checked">
                </s:if>
                <s:else>
                   		<input id="scenery_recommend" type="checkbox" name="scenery_recommend">
                </s:else>   
				<label for="scenery_recommend">推荐</label>
		      </td>
		    </tr>
		    <tr>
		      <td></td>
